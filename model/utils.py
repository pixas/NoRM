...
class MoLoRALinear(nn.Linear, LoRALayer):
    ...
    def molora_helper3(self, x: torch.Tensor):
        ...
        if self.share_expert:
            ...
            temp_results = torch.stack([expert[f'lora_B_{i}'](expert[f'lora_A_{i}'](x)) * self.scaling if i < self.num_experts else expert(x) for i, expert in enumerate(self.experts)], dim=0)  # [expert, bs * N, out_features]
...
def multiple_path_forward(self, input_ids: torch.LongTensor = None,
    ...
    output_logit_loss = output_logit_loss if output_logit_loss is not None else self.config.output_logit_loss
    ...
    if labels is not None:
        ...
        if inference_path <= 1:
            ...
            if logit_sum != ():
                # first construct a L * N * bs * seq_length tensor
                router_loss = ROUTER_LOSS_MAPPING[output_logit_loss](logit_sum, labels, minimum_path_index, inference_path, length)
